...
constexpr std::array<uint8_t, 256> CDRomDrive::ExpectedCommandParameters = []
{
...
uint8_t CDRomDrive::Read( uint32_t registerIndex ) noexcept
{
	switch ( registerIndex )
	{
		case 0:
		{
			...
			CDROMDRIVE_TRACE( "CDRomDrive::Read -- status [%02X]", m_status.value );
		...
		case 1: // response FIFO (all indices)
		{
			if ( !m_responseBuffer.Empty() )
			{
				...
				CDROMDRIVE_TRACE( "CDRomDrive::Read -- response FIFO [%02X]", value );
				UpdateStatus();
				return value;
			}
			else
	...
	switch ( registerIndex )
	{
		...
		case 1:
			switch ( m_status.index )
			{
				case 0: // command register
					CDROMDRIVE_TRACE( "CDRomDrive::Write -- send command [%02X]", value );
					SendCommand( static_cast<Command>( value ) );
					break;

				case 1: // sound map data out
					dbLogWarning( "CDRomDrive::Write -- ignoring sound map data out [%02X]", value );
					break;

				case 2: // sound map coding info
					dbLogWarning( "CDRomDrive::Write -- ignoring sound map coding info [%02X]", value );
					break;

				case 3: // audio volume for right-cd-out to right-spu-input
					CDROMDRIVE_TRACE( "CDRomDrive::Write -- right-cd-out to right-spu-input [%02X]", value );
		...
		case 2:
			switch ( m_status.index )
			{
				case 0: // parameter fifo
					CDROMDRIVE_TRACE( "CDRomDrive::Write -- parameter [%02X]", value );
					m_parameterBuffer.Push( value );
					UpdateStatus();
					break;

				case 1: // interrupt enable
					CDROMDRIVE_TRACE( "CDRomDrive::Write -- interrupt enable [%02X]", value );
					m_interruptEnable = value & InterruptFlag::WriteMask;
					CheckInterrupt();
					break;

				case 2: // left-cd-out to left-spu-input
					CDROMDRIVE_TRACE( "CDRomDrive::Write -- left-cd-out to left-spu-input [%02X]", value );
					m_nextVolumes.leftToLeft = value;
					break;

				case 3: // right-cd-out to left-cd-input
					CDROMDRIVE_TRACE( "CDRomDrive::Write -- right-cd-out to left-cd-input [%02X]", value );
		...
		case 3:
			switch ( m_status.index )
			{
				...
				{
					CDROMDRIVE_TRACE( "CDRomDrive::Write -- data request [%02X]", value );
				...
				{
					CDROMDRIVE_TRACE( "CDRomDrive::Write -- interrupt flag [%02X]", value );
				...
				{
					CDROMDRIVE_TRACE( "CDRomDrive::Write -- left-cd-out to right-spu-input [%02X]", value );
				...
				{
					CDROMDRIVE_TRACE( "CDRomDrive::Write -- audio volume apply [%02X]", value );
...
void CDRomDrive::DmaRead( uint32_t* data, uint32_t count )
{
	...
	else if ( m_dataBuffer.Size() > requestedBytes )
	{
		dbLogWarning( "CDRomDrive::DmaRead -- %u bytes remaining", m_dataBuffer.Size() - requestedBytes );
		dbBreak();
	}
...
cycles_t CDRomDrive::GetSeekCycles( CDRom::LogicalSector seekPosition ) const noexcept
{
	...
	if ( sectorDiff < 32 )
	{
		static constexpr uint32_t MinSectorDiff = 5;
	...
	constexpr bool ignoreSpeedChange = false;
	if ( m_driveState == DriveState::ChangingSpeedOrReadingTOC && !ignoreSpeedChange )
	{
		...
		CDROMDRIVE_TRACE( "Seek time for %u sectors: %d (%.3f ms) (%d for speed change/implicit TOC read)", sectorDiff, cycles,
			( static_cast<float>( cycles ) / static_cast<float>( CpuCyclesPerSecond ) ) * 1000.0f, remainingChangeCycles );
	}
	else
	{
		CDROMDRIVE_TRACE( "Seek time for %u sectors: %d (%.3f ms)", sectorDiff, cycles,
...
void CDRomDrive::ExecuteCommand() noexcept
{
	...
	switch ( command )
	{
		...
		case Command::Pause:
		{
			...
			if ( IsSeeking() )
			{
				// Duckstation: On console, this returns an error. But perhaps only during the coarse/fine seek part? Needs more hardware tests.
...
void CDRomDrive::RequestData() noexcept
{
	...
	// DEBUG LOG
	CDROMDRIVE_TRACE( "Requesting data for CD sector %u", sector.sector );
...
void CDRomDrive::ProcessDataSector( const CDRom::Sector& sector )
{
	...
	// DEBUG
	buffer.sector = m_currentPosition;
